...
# --- Define path to the local models directory ---
MODELS_DIR = "models" 
SPACY_MODEL_NAME = "en_core_web_sm"
SPACY_MODEL_PATH = os.path.join(MODELS_DIR, SPACY_MODEL_NAME)

# --- Global SpaCy NLP Model Loading ---
# Load the pre-downloaded spaCy model from the local 'models' folder.
try:
    nlp = spacy.load(SPACY_MODEL_PATH)
    print(f"Successfully loaded local spaCy model from: {SPACY_MODEL_PATH}")
except OSError:
    print(f"Error: Local spaCy model not found at '{SPACY_MODEL_PATH}'.")
    print("Please run the 'downloader.py' script first to download the models.")
    exit(1)
...
def compute_features(span_data_list):
    ...
    for span in span_data_list:
        ...
        # For now, I'll add placeholders or simple heuristics if direct extraction is not clear.
...
try:
    ...
    print("All models and scalers/encoders loaded successfully.")
...
def predict_document_outline(pdf_path):
    ...
    print(f"DEBUG: Extracted {len(raw_spans)} raw spans.")
    if len(raw_spans) > 0:
        print(f"DEBUG: First 3 raw spans (text, font_size, is_bold, bbox):")
        for i, span in enumerate(raw_spans[:3]):
            print(f"  {i}: Text='{span['text']}', Size={span['font_size']:.2f}, Bold={span['is_bold']}, Bbox=({span['bbox']['x0']:.2f}, {span['bbox']['y0']:.2f}, {span['bbox']['x1']:.2f}, {span['bbox']['y1']:.2f})")
    else:
        print(f"DEBUG: No text spans found in {pdf_path}.")
        return {"title": "", "outline": []}

    # Step 2: Compute rich features for each span
    featured_spans = compute_features(raw_spans)
    print(f"DEBUG: Computed features for {len(featured_spans)} spans.")
    if len(featured_spans) > 0:
        print("DEBUG: Features for first span:")
        # Print only a subset of features to avoid overwhelming output, or the whole dict if small
        # print(json.dumps(featured_spans[0]['features'], indent=2))
        sample_features = {k: featured_spans[0]['features'][k] for k in list(featured_spans[0]['features'].keys())[:10]} # Print first 10
        print(f"  Sample features for first span: {sample_features}...")
    ...
    if not features_data:
        print(f"DEBUG: No valid features extracted from {pdf_path}. Returning empty outline.")
        return {"title": "", "outline": []}
    ...
    X = X.fillna(X.mean(numeric_only=True)) # Added numeric_only=True for newer pandas versions

    print(f"DEBUG: DataFrame X shape for prediction: {X.shape}")
    print("DEBUG: First 5 rows of feature DataFrame X (before scaling):")
    print(X.head())
    ...
    # >>> CRITICAL FIX HERE <<<
    # is_heading_label_encoder.inverse_transform will return True/False directly.
    # No need to compare with string "Heading".
    ...
    is_heading_status = is_heading_predictions # FIX: Changed from (is_heading_predictions == "Heading")
    print("DEBUG: is_heading_predictions (raw after inverse_transform):") # Label changed for clarity
    print(is_heading_predictions) # Print the actual values
    print("DEBUG: is_heading_status (boolean - corrected):") # Label changed for clarity
    print(is_heading_status)
    ...
    print(f"DEBUG: Number of spans predicted as 'Heading': {len(heading_spans_indices)}")

    if len(heading_spans_indices) > 0:
        X_headings = X.iloc[heading_spans_indices]
        
        print(f"DEBUG: DataFrame X_headings shape for level prediction: {X_headings.shape}")
        print("DEBUG: First 5 rows of heading feature DataFrame X_headings (before scaling):")
        print(X_headings.head())
        ...
        print("DEBUG: heading_level_predictions (raw):")
        print(heading_level_predictions)
        ...
        for i, span in enumerate(featured_spans):
            if is_heading_status[i]: # If Model 1 predicted it's a heading
                ...
                print(f"DEBUG: Span {i} ('{span['text'][:50]}...') predicted as HEADING, level: {predicted_level}")
                
                # Check for Title
                if predicted_level == "Title" and not title_found:
                    document_title = span['text']
                    title_found = True
                    print(f"DEBUG: Identified document title: '{document_title}'")
                    ...
                    print(f"DEBUG: Added to outline: Level={predicted_level}, Text='{span['text'][:50]}...', Page={span['page_number']}")
                else:
                    print(f"DEBUG: Heading at span {i} with text '{span['text'][:50]}...' was not H1, H2, H3, or Title, so not added to outline. Predicted Level: {predicted_level}")
            # else: it's not a heading, so we ignore it for the outline

    print(f"DEBUG: Final document_title: '{document_title}'")
    print(f"DEBUG: Final outline (number of entries): {len(document_outline)}")
...
if __name__ == "__main__":
    ...
    INPUT_DIR = "pdfs"  # Change to "/app/input" for Docker submission
    OUTPUT_DIR = "r1_op" # Change to "/app/output" for Docker submission
    # >>> TEMPORARY LOCAL TESTING PATHS - REMEMBER TO CHANGE BACK FOR DOCKER! <<<
